...
struct song
{
    ...
    song(char name[50], string author, int duration) : name(name), author(author), duration(duration) {}
};
...
void selectionSort(Node *node)
{
    ...
    while (aux->right != NULL)
    {
        ...
        {
            char value1[50];
            strcpy(value1, aux2->value.name);
            char value2[50];
            strcpy(value2, aux3->value.name);
            if (strcmp(aux2->value.name, aux3->value.name) > 0)
            {
...
void sortPlaylist(Node *node)
{
    ...
    for (i = node->right; i != NULL; i = i->right)
    {
        key = i->value;
        j = i->left;
        while (j != NULL && strcmp(j->value.name.c_str(), key.name.c_str()) > 0)
        {
